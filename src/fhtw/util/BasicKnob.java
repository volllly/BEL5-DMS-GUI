...
public class BasicKnob extends Region
{
	...
	private double maxAngle = 360 - 45;
	...
	public BasicKnob(String styleClass, String knobTheme, double knobSize)
	{
		...
		themeAccess.setOnMouseDragged(new EventHandler<MouseEvent>()
		{
			...
			public void handle(MouseEvent event) {
				...
				angle = Math.max(getMinAngle(), Math.min(getMaxAngle(), angle));
	...
	protected void layoutChildren() {
		...
		double centerX = knob.getWidth() / 2.0;
		double centerY = knob.getHeight() / 2.0;

		minLine.setStartX(centerX);
		minLine.setStartY(centerY);
		minLine.setEndX(centerX + knobSize * Math.cos(Math.toRadians(-minAngle)));
		minLine.setEndY(centerY + knobSize * Math.sin(Math.toRadians(-minAngle)));
		...
		double knobX = (knob.getWidth() - knob.getPrefWidth()) / 2.0;
		double knobY = (knob.getHeight() - knob.getPrefHeight()) / 2.0;
		...
		if (minAngle <= angle && angle <= maxAngle && withinAngle) {
			...
			if(initThemePos) {
				rotate.setAngle(-minAngle / 2 - 90);
				initThemePos = false;
			} else {
				rotate.setAngle(-angle / 2 - 90);
			}
		}
	}
...
//		maxLine.setStroke(Color.BLACK);
	...
	public void setKnobSize(double s) {
		knobSize = s;
		knob.setPrefSize(this.knobSize,this.knobSize);
		knob.getTransforms().add(rotate);
	}
